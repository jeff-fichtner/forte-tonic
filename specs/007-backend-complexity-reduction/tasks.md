...
- [x] T010 [P] [US2] Replace `AppConfigurationResponse.fromApiData(data)` call in `src/web/js/viewModel.ts:28` with `new AppConfigurationResponse(data)`. Then remove the `fromApiData` static method from `src/models/shared/responses/appConfigurationResponse.ts`
