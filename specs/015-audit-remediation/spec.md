...
| Explicit `x-login-type: employee` header does NOT skip the parent lookup for a 10-digit code (surfaced by 015 US6 — format heuristic still fires parent lookup first; a 10-digit code matching a parent gets the parent identity even when the caller asked for employee auth) | [016-error-contract-uniformization](../016-error-contract-uniformization/spec.md) |
