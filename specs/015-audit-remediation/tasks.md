...
- [ ] T033 [US4] DEFERRED to pre-merge verification by user. Driving a live browser session (login as parent, switch via "Change User" to employee, check console) is not something I can do autonomously, and there is no jsdom-based DOM test infrastructure yet (routed to spec 019). Type-system + integration tests cover the compile-time and API-level contract; the human smoke test before merge covers the runtime DOM behavior.
