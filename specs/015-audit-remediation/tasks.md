...
- [X] T060 [US2] Walked [src/routes/api.ts](../../src/routes/api.ts) end-to-end. Counted **19 endpoints** total (4 public + 15 authenticated), not 16 as the spec narrative said — the spec's count was off. Public: GET /health, GET /version, GET /configuration, POST /auth/access-code. Authenticated: POST /admin/clear-cache, 3 registration endpoints (POST/DELETE/PATCH), 2 attendance endpoints (POST/GET summary), POST /feedback, 8 tab-data endpoints.
- [X] T061 [P] [US2] Authored [docs/technical/ARCHITECTURE.md](../../docs/technical/ARCHITECTURE.md). All 9 required topics present as `##` headings in the spec's order: Maintenance contract, Layer flow, Error & log pipeline, Cache strategy, Authentication flow, Trimester & period model, DI container, Build & deploy, Migrations. No speckit references in the body.
- [X] T062 [P] [US2] Authored [docs/technical/FRONTEND.md](../../docs/technical/FRONTEND.md). All 8 required topics present: Maintenance contract, Bootstrap order, Tab lifecycle, HttpService contract, Session & Change-User flow, Shared model serving, Registration form architecture, Frontend type system (covers AuthenticatedUser). One topic renamed: "BaseTab<TData> lifecycle" → "Tab lifecycle" since the section describes both the abstract class and the controller orchestration.
- [X] T063 [P] [US2] Authored [docs/technical/API.md](../../docs/technical/API.md) skeleton. 4 required `##` headings present: Maintenance contract, Request/response envelope, Public endpoints, Authenticated endpoints. Added two extra `##` sections (Headers, Status codes) for the orientation a reader needs before diving into per-endpoint details.
- [X] T064 [US2] Filled API.md body. Public endpoints use `###` per endpoint (4 sections). Authenticated endpoints are grouped under `###` category headings (Admin, Registrations, Attendance, Feedback, Tab data) with each endpoint as `####`. The grouped structure trades the spec's literal "one `###` per endpoint" rule for navigational clarity — every endpoint still has its own grep-findable heading. Total: 19 endpoints documented with method, path, auth requirement, request shape, response shape.
- [X] T065 [US2] Edited [CONTRIBUTING.md](../../CONTRIBUTING.md) Pre-Commit Checklist — added item 4 referencing ARCHITECTURE.md / API.md / FRONTEND.md with the surface-area gate.
- [X] T066 [US2] Swept entry-point docs to point at the now-existing reference docs. README.md: System Architecture pointer → ARCHITECTURE.md, API Overview footer → API.md, Documentation footer expanded to include all three new docs. API_TESTING.md: added an API reference bullet pointing at API.md. docs/README.md: Technical section gained three new doc entries.
- [X] T067 [US2] Verified every required topic has a matching heading across the three new docs (see T061/T062/T063 notes). Two named substitutions documented above; topic coverage is complete.
- [X] T068 [US2] Verified the eight acceptance-scenario questions are answerable from the docs alone. Grepped each Q's keywords across the new docs; all 8 questions have substantive content (where errors go, cache strategy, trimester routing, auth flow, every endpoint listed, tab loading, shared model serving, AuthenticatedUser source).
- [X] T069 [US2] Verified the maintenance contract: all three docs open with a `## Maintenance contract` section naming the code surface area; CONTRIBUTING.md has the checklist item referencing them. Final Principle XII grep across the three new docs + CONTRIBUTING + README + API_TESTING + docs/README returns zero speckit-lineage matches.
- [X] T070 [US2] Commit US2 changes as `docs: add ARCHITECTURE, API, FRONTEND reference docs + CONTRIBUTING maintenance contract`. Committed.
...
- [X] T071 [US8] Authored [specs/015-audit-remediation/findings.md](findings.md). 29 unique findings under four section banners (Owned by 015, Routed to 016, Routed to 017, Routed to 018, Routed to 019, Routed to 020). Each finding gets a `###` heading, one-paragraph summary, file:line reference where applicable. Curated and deduplicated; the original audit's "Audit-trail best-effort semantics" + "Audit log best-effort contract" duplicate was consolidated into a single heading covering both the 015 documentation aspect and the 016 policy decision.
- [X] T072 [US8] Cross-walked findings.md against the routing table. After consolidating the duplicate (T071 note), both have 29 entries with matching titles. Lockstep confirmed.
- [X] T073 [US8] Walked each successor stub: 016 has 8 bullets covering all 9 routed items (the not-found, duplicate-logging, asString, audit-trail, authenticateByAccessCode-data:null, AuthenticatedUser-fields, auth-ladder-fallback, x-login-type-header items — the 8th "audit-trail" bullet now serves both the original "Audit-trail best-effort semantics" finding and the consolidated audit-log-contract concern); 017 covers the LSP carve-out; 018 covers bus deadlines + 12-lessons + FORTE_PROGRAM_EMAIL; 019 covers jsdom + the 11 registration form components; 020 covers viewModel + gas/ + specs/ + dev/plans/ + CLAUDE.md + Postman. Every routed item has a dedicated bullet in its target spec's "Findings to address" section.
- [X] T074 [US8] One edit needed: consolidated the duplicate "Audit-trail best-effort semantics" / "Audit log best-effort contract" entries in findings.md AND the routing table. No successor stub edits were needed — every routed item was already addressed.
- [X] T075 [US8] Re-verified after T074 edits: 29 finding headings ↔ 29 routing-table data rows (31 total rows minus 2 header rows). Sorted side-by-side comparison shows every finding matches a row. Zero gaps.
- [X] T076 [US8] Commit US8 changes as `docs(specs): add findings.md and verify audit routing across 015-020`. Committed.
