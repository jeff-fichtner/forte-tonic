...
describe('Value Object ID Tests', () => {
  ...
  describe('Practical usage scenarios', () => {
    ...
    test('should work correctly in array some operations', () => {
      ...
      // Incorrect way (the bug)
      const hasRegistrationsIncorrect = registrations.some(x => x.instructorId === targetInstructorId);
      expect(hasRegistrationsIncorrect).toBe(false);
      
      // Correct way (the fix)
      const hasRegistrationsCorrect = registrations.some(x => x.instructorId.value === targetInstructorId);
