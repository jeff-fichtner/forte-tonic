...
export class StudentService {
  ...
  async getStudents(searchRequest) {
    try {
      ...
      const apiStudents = result.students.map(dbStudent => ApiStudent.fromDatabase(dbStudent));

      return new StudentListResponse(apiStudents, result.totalCount, result.page, result.pageSize);
    } catch (error) {
  ...
  async getStudentsByParent(parentId) {
    try {
      ...
      const apiStudents = dbStudents.map(dbStudent => ApiStudent.fromDatabase(dbStudent));
  ...
  async getAvailableInstructors(studentId, instrument = null, day = null) {
    try {
      const instructors = await this.unitOfWork.findAvailableInstructors(
        studentId,
        instrument,
        day
      );
