...
class MasterScheduleTable {
  ...
  async getAttendanceSummary(registrationId) {
    try {
      const response = await fetch(
        `/api/attendance/summary/${registrationId}?schoolYear=2025-2026&trimester=Fall`
      );
  ...
  updateRowWithRealData(tempId, realData) {
    ...
    row.querySelector('td:nth-child(2)').textContent =
      realData.className || realData.instructorName;
  ...
  markAttendanceOptimistic(registrationId, week) {
    ...
    if (row) {
      const attendanceIndicator =
        row.querySelector('.attendance-indicator') || this.createAttendanceIndicator(row);
